...
def _write_identity_marker(
    staged_root: Path, product_id: str, profile: dict[str, str]
) -> str:
...
def _bind_identity_marker(
    staged_root: Path, main_scene: str, marker_resource: str
) -> None:
    scene_path = staged_root / main_scene.removeprefix("res://")
    scene_text = scene_path.read_text(encoding="utf-8")
    marker_declaration = (
        f'[ext_resource type="Resource" path="{marker_resource}" id="2_identity"]\n'
    )
...
def _validate_staging_target(
    repository_root: Path, staged_root: Path
) -> tuple[Path, Path]:
    canonical_root = (repository_root / "godot/Tet4D.Godot").resolve()
    if _inside(staged_root, canonical_root):
        raise ProfileError(
            "profile staging target must be outside the canonical Godot project"
        )
...
def _apply_project_profile(
    project_path: Path, staged_root: Path, product_id: str, profile: dict[str, str]
) -> None:
    for resource in (profile["main_scene"], profile["icon"]):
        relative = resource.removeprefix("res://")
        if (
            resource == relative
            or not _inside(staged_root / relative, staged_root)
            or not (staged_root / relative).is_file()
        ):
...
def _apply_preset_profile(presets_path: Path, profile: dict[str, str]) -> None:
    ...
    for section in parser.sections():
        ...
        platform = (
            parser[section.removesuffix(".options")].get("platform", "").strip('"')
        )
        if platform in {"macOS", "iOS"} and "application/bundle_identifier" in options:
            options["application/bundle_identifier"] = (
                f'"{profile["application_identity"]}"'
            )
...
def apply_profile(repository_root: Path, staged_root: Path, product_id: str) -> None:
    ...
    profile = load_profile(
        repository_root / "config/project/policy_pack.json", product_id
    )
...
def validate_staged_profile(
    repository_root: Path, staged_root: Path, product_id: str
) -> None:
    repository_root = repository_root.resolve()
    staged_root = staged_root.resolve()
    ...
    for key, value in expected:
        if f'{key}="{value}"' not in project_text:
            raise ProfileError(
                "staged project identity does not match the selected profile"
            )
    marker_path = staged_root / identity_marker_resource(product_id).removeprefix(
        "res://"
    )
    try:
        ...
        raise ProfileError(
            "staged project identity marker is missing or invalid"
        ) from exc
    if marker != {
        "product_id": product_id,
        "application_identity": profile["application_identity"],
        ...
        raise ProfileError(
            "staged project identity marker does not match the selected profile"
        )
    scene_text = (staged_root / profile["main_scene"].removeprefix("res://")).read_text(
        encoding="utf-8"
    )
    if (
        f'path="{identity_marker_resource(product_id)}"' not in scene_text
        or 'metadata/tet4d_product_identity_marker = ExtResource("2_identity")'
        not in scene_text
    ):
...
def main() -> int:
    ...
    try:
        if args.validate:
            validate_staged_profile(
                args.repository_root, args.staged_root, args.product
            )
        else:
