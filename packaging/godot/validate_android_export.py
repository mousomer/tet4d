...
ABSOLUTE_PATH_PATTERN = re.compile('"(?:' + "|".join(_HOME_PREFIXES) + r"|[A-Za-z]:\\)")
    ...
    for directory, subdirectories, filenames in os.walk(repository_root):
        ...
        for filename in filenames:
            if filename.endswith(SIGNING_SECRET_SUFFIXES):
                found.append(
                    str(Path(directory, filename).relative_to(repository_root))
                )
    return found
...
def _presets(repository_root: Path) -> tuple[configparser.ConfigParser, str]:
    ...
    parser.read(
        repository_root / "godot/Tet4D.Godot/export_presets.cfg", encoding="utf-8"
    )
...
def _check_identity(
    options: configparser.SectionProxy, preset: configparser.SectionProxy, version: str
) -> None:
...
def _check_tablet_shape(options: configparser.SectionProxy) -> None:
    ...
    if (
        options.get("screen/support_large") != "true"
        or options.get("screen/support_xlarge") != "true"
    ):
        raise AndroidExportError(
            "Android tablet build must declare large screen support"
        )
    if (
        options.get("screen/support_small") != "false"
        or options.get("screen/support_normal") != "false"
    ):
        raise AndroidExportError(
            "Android tablet build must not advertise handset screens"
        )
    ...
    if options.get("gradle_build/use_gradle_build") != "false":
        raise AndroidExportError(
            "Android export must use the prebuilt template, not a Gradle build"
        )
...
def _check_project_settings(repository_root: Path) -> None:
    ...
    if orientation not in LANDSCAPE_ORIENTATIONS:
        raise AndroidExportError(
            f"handheld orientation must be landscape, not {orientation!r}"
        )
...
def _check_no_committed_secrets(
    repository_root: Path, options: configparser.SectionProxy
) -> None:
    for option in SIGNING_SECRET_OPTIONS:
        if options.get(option, '""').strip('"'):
            raise AndroidExportError(
                f"signing material must not be committed in {option}"
            )
    ...
    if ABSOLUTE_PATH_PATTERN.search(export_text):
        raise AndroidExportError(
            "export presets must not depend on a development machine path"
        )
...
def _check_pack(pack_path: Path) -> int:
    ...
    if missing:
        raise AndroidExportError(
            f"Android pack is missing required resources: {missing}"
        )
    ...
    if development_only:
        raise AndroidExportError(
            f"development-only files entered the Android pack: {development_only[:5]}"
        )
    return len(paths)
...
def main() -> int:
    ...
    parser.add_argument(
        "--pack", type=Path, default=None, help="exported Android .pck to inspect"
    )
    args = parser.parse_args()
    try:
        result = validate(
            args.repository_root.resolve(), args.pack.resolve() if args.pack else None
        )
