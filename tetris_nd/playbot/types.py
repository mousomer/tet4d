...
def _budget_table_for_ndim(ndim: int) -> tuple[int, int, int]:
    return playbot_budget_table_for_ndim(ndim)


def default_planning_budget_ms(ndim: int, profile: BotPlannerProfile) -> int:
    fast, balanced, deep = _budget_table_for_ndim(ndim)
    if profile == BotPlannerProfile.FAST:
        return fast
    if profile == BotPlannerProfile.DEEP:
        return deep
    return balanced


def clamp_planning_budget_ms(ndim: int, budget_ms: int) -> int:
    floor_divisor, floor_min, ceil_multiplier, ceil_min = playbot_clamp_policy()
    floor = max(
        floor_min,
        default_planning_budget_ms(ndim, BotPlannerProfile.FAST) // max(1, floor_divisor),
    )
    ceil = max(
        ceil_min,
        default_planning_budget_ms(ndim, BotPlannerProfile.DEEP) * max(1, ceil_multiplier),
    )
    return max(floor, min(ceil, int(budget_ms)))


def planning_lookahead_depth(ndim: int, profile: BotPlannerProfile) -> int:
    return playbot_lookahead_depth(ndim, profile.value)


def planning_lookahead_top_k(ndim: int, profile: BotPlannerProfile) -> int:
