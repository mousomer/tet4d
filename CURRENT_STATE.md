...
- Explorer compatibility-mirror cleanup + legacy-panel decision pass (2026-03-22): `src/tet4d/ui/pygame/topology_lab/scene_state.py` now rebuilds canonical playground state from canonical selectors rather than stale shell mirror inputs, retained shell fields are synchronized compatibility projections instead of explorer-path truth inputs, dirty-state sync now routes through explicit helpers, and the old `legacy_panel_support.py` bucket is retired in favor of the narrower transitional seam `src/tet4d/ui/pygame/topology_lab/legacy_normal_mode_support.py` for legacy Normal Game rows plus resolved-profile export only.
...
- Legacy-consumer retirement follow-up (2026-03-12, narrowed 2026-03-22): the migrated probe/highlight path now writes canonical playground probe state through `src/tet4d/ui/pygame/topology_lab/scene_state.py` helpers, and the remaining Normal Game rows plus resolved-profile export now sit behind the explicitly transitional seam `src/tet4d/ui/pygame/topology_lab/legacy_normal_mode_support.py` instead of the vaguer `legacy_panel_support.py` bucket.
...
- Remaining compatibility debt after this pass: retained shell fields in `src/tet4d/ui/pygame/topology_lab/scene_state.py` now survive only as synchronized compatibility projections for legacy readers/diagnostics, not as explorer-path input authority; normal-mode legacy rows and resolved-profile export remain intentionally isolated in `src/tet4d/ui/pygame/topology_lab/legacy_normal_mode_support.py`; and the legacy inspect naming seam is now limited to accepting the serialized/input token `inspect_boundary` through runtime tool normalization for older saved state/input callers.
...
1. `tests/unit/engine/test_topology_lab_menu.py`: `2985` real LOC
2. `scripts/arch_metrics.py`: `1869` real LOC
3. `src/tet4d/ui/pygame/topology_lab/controls_panel.py`: `1526` real LOC
4. `src/tet4d/engine/tutorial/setup_apply.py`: `1496` real LOC
5. `tools/governance/validate_project_contracts.py`: `1177` real LOC
6. `src/tet4d/ui/pygame/topology_lab/scene_state.py`: `1069` real LOC
