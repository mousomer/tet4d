...
- Explorer probe-mirror retirement + legacy-row decision pass (2026-03-22): `src/tet4d/ui/pygame/topology_lab/scene_state.py` no longer re-synchronizes the probe shell mirror trio (`probe_coord`, `probe_trace`, `probe_path`) from canonical state, so canonical selectors are the only live explorer-path readers while those fields remain fallback-only compatibility storage when canonical state is absent; the old `legacy_panel_support.py` follow-up is now complete because `src/tet4d/ui/pygame/topology_lab/legacy_normal_mode_support.py` is retired and the last legacy Normal Game row-adjustment helper now lives narrowly inside `controls_panel.py`.
...
1. `tests/unit/engine/test_topology_lab_menu.py`: `3098` real LOC
2. `scripts/arch_metrics.py`: `1869` real LOC
3. `src/tet4d/ui/pygame/topology_lab/controls_panel.py`: `1646` real LOC
4. `src/tet4d/engine/tutorial/setup_apply.py`: `1496` real LOC
5. `tools/governance/validate_project_contracts.py`: `1177` real LOC
6. `src/tet4d/ui/pygame/topology_lab/scene_state.py`: `1059` real LOC
