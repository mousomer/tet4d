...
build_config = frontend_nd_setup.build_play_menu_config
...
def _capture_endgame_snapshot_3d(
    ...
    (
        preset_id,
        _legacy_boundary_response,
        _legacy_particle_collisions,
        relic_speed_percent,
    ...
    locked_cells = tuple(
        SnapshotCell(
            ...
            position=(float(coord[0]), float(coord[1]), float(coord[2])),
    ...
    return create_snapshot(
        ...
        render_dims=tuple(int(value) for value in loop.cfg.dims),
        locked_cells=locked_cells,
        base_seed=int(loop.cfg.rng_seed),
        render_context=EndgameRenderContext(
            mode_key="3d",
            projection_name=str(loop.camera.projection.name),
            yaw_deg=float(loop.camera.yaw_deg),
            pitch_deg=float(loop.camera.pitch_deg),
            zoom=float(loop.camera.zoom),
            cam_dist=float(loop.camera.cam_dist),
            projective_strength=float(loop.camera.projective_strength),
            projective_bias=float(loop.camera.projective_bias),
        ),
        preset_id=preset_id,
        boundary_response=explosion_defaults.boundary_response,
        particle_collisions=explosion_defaults.particle_collisions,
        ...
        diagnostics_mode=explosion_defaults.diagnostics_mode,
        trace_retention_ms=explosion_defaults.trace_retention_ms,
        speed_preset=explosion_defaults.speed_preset,
        grid_mode=explosion_defaults.grid_mode,
        shadow_mode=explosion_defaults.shadow_mode,
        trace_enabled=explosion_defaults.trace_enabled,
        w_movement_animation_style=explosion_defaults.w_movement_animation_style,
        ...
        return cls(
            ...
            rotation_anim=PieceRotationAnimatorND(
                ...
                duration_ms=(
                    ...
                    else PieceRotationAnimatorND(ndim=3, gravity_axis=1).duration_ms
                ),
...
def run_game_loop(
    ...
    *,
    ...
    rotation_animation_duration_ms, translation_animation_duration_ms = (
        mode_animation_settings("3d")
    )
    rotation_animation_mode = mode_rotation_animation_mode("3d")
