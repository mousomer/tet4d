...
class EndgamePreviewFrame:
    ...
    shard_frames: tuple[
        tuple[EndgameBoundaryImpact, EndgameBoardShard, EndgameShardDrawState], ...
    ]
...
def default_shell_preview_time_scale(tuning: dict[str, object] | None = None) -> float:
    ...
    return max(
        0.0, _float_value(resolved_tuning, "shell_preview_default_time_scale", 1.0)
    )
...
def _lerp_vec(
    start: tuple[float, ...], end: tuple[float, ...], t: float
) -> tuple[float, ...]:
    progress = _clamp01(t)
...
def shell_preview_phase_for_elapsed(
    elapsed_ms: float, tuning: dict[str, object] | None = None
) -> str:
    resolved_tuning = _tuning() if tuning is None else tuning
    ...
    *,
    ...
    if phase == _PHASE_HOLD:
        ...
        duration_ms = _float_value(
            resolved_tuning, "shell_preview_shard_drift_ms", 2200.0
        )
    else:
        return 1.0
...
def scaled_shell_preview_elapsed(
    state, dt_ms: float, tuning: dict[str, object] | None = None
) -> float:
    resolved_tuning = _tuning() if tuning is None else tuning
    default_scale = _float_value(
        resolved_tuning, "shell_preview_default_time_scale", 1.0
    )
    scale = float(getattr(state, "shell_preview_time_scale", default_scale))
    return max(0.0, float(dt_ms)) * max(0.0, scale)


def _sorted_seed_cells(
    ...
    return tuple(
        sorted(
            source_cells,
            key=lambda cell: (
                tuple(int(v) for v in cell.source_coord),
                int(cell.color_id),
                str(cell.source_group_id or ""),
            ),
        )
    )


def _preview_source_cells(
    source_cells: tuple[ExplosionSeedCell, ...], dimension: int
) -> tuple[PreviewSourceCell, ...]:
    return tuple(
        PreviewSourceCell(
            ...
            layer_index=int(cell.source_coord[3])
            if int(dimension) == 4 and len(cell.source_coord) > 3
            else None,
            source_group_id=str(cell.source_group_id)
            if cell.source_group_id is not None
            else None,
        )
...
def _seed_cells(
    snapshot_cells: tuple[SnapshotCell, ...],
    *,
    source_lookup: dict[tuple[int, ...], PreviewSourceCell],
) -> tuple[ExplosionSeedCell, ...]:
    return tuple(
        ExplosionSeedCell(
            source_coord=tuple(int(v) for v in cell.source_coord),
            color_id=int(cell.color_id),
            source_group_id=source_lookup[
                tuple(int(v) for v in cell.source_coord)
            ].source_group_id,
        )
...
def preview_source_cell_map(
    cache: EndgamePreviewCache,
) -> dict[tuple[int, ...], PreviewSourceCell]:
    return {
        tuple(int(v) for v in cell.source_coord): cell for cell in cache.source_cells
    }
...
def advance_shell_preview_elapsed(state, dt_ms: float) -> None:
    if bool(getattr(state, "shell_preview_enabled", False)):
        ...
        state.shell_preview_elapsed_ms = max(
            0.0,
            float(getattr(state, "shell_preview_elapsed_ms", 0.0))
            + scaled_shell_preview_elapsed(state, dt_ms, tuning),
        )
...
def _build_cache(
    ...
    source_lookup = {
        tuple(int(v) for v in cell.source_coord): cell for cell in preview_cells
    }
    snapshot_cells = _snapshot_cells(preview_cells)
    split = split_endgame_locked_cells(
        locked_cells=snapshot_cells,
        dimension=int(dimension),
        seed=int(seed),
        live_fraction=float(live_fraction),
    )
    escaping_source_cells = tuple(
        source_lookup[tuple(int(v) for v in cell.source_coord)]
        for cell in split.escaping_cells
    )
...
def ensure_shell_preview_cache(
    state, *, source_cells: tuple[ExplosionSeedCell, ...], board_dims: tuple[int, ...]
) -> EndgamePreviewCache | None:
    ...
    signature = _signature(
        tuple(source_cells),
        board_dims,
        int(getattr(state, "dimension", len(board_dims))),
        int(getattr(state, "seed", 0)),
        float(getattr(state, "endgame_live_cell_fraction", 0.12)),
    )
    if (
        getattr(state, "shell_preview_cache", None) is None
        or getattr(state, "shell_preview_signature", None) != signature
    ):
        state.shell_preview_elapsed_ms = 0.0
        state.shell_preview_signature = signature
        state.shell_preview_cache = _build_cache(
            tuple(source_cells),
            tuple(int(v) for v in board_dims),
            int(getattr(state, "dimension", len(board_dims))),
            int(getattr(state, "seed", 0)),
...
def preview_survivor_cells_for_state(
    state, *, source_cells: tuple[ExplosionSeedCell, ...], board_dims: tuple[int, ...]
) -> tuple[ExplosionSeedCell, ...]:
    cache = ensure_shell_preview_cache(
...
def _rupture_impact_draw_state(
    ...
    *,
    ...
    eased = 0.08 + (
        0.92 * (0.5 - (0.5 * math.cos(math.pi * _clamp01(rupture_progress))))
    )
    ...
    return EndgameImpactDrawState(
        ...
        streak_width=_float_value(tuning, "impact_streak_width", 3.5)
        * (0.9 + (0.55 * eased)),
    )
...
def _rupture_proxy_alpha(
    impact: EndgameBoundaryImpact, *, rupture_progress: float
) -> float:
...
def _rupture_proxy_rotation_deg(
    ...
    *,
    ...
    spin = (
        float(impact.side) * (18.0 + (54.0 * float(impact.force))) * (0.35 + progress)
    )
...
def _escaping_proxy_state_for_impact(
    ...
    *,
    ...
    return ShellPreviewEscapingCellState(
        ...
        render_position=_lerp_vec(
            impact.start_position, impact.impact_position, progress
        ),
...
def _residue_frames(
    ...
    *,
    ...
    if phase == _PHASE_RUPTURE:
        ...
        progress = shell_preview_timeline_progress(
            elapsed_ms, phase=_PHASE_SHARD_DRIFT, tuning=tuning
        )
        alpha = _float_value(tuning, "cracked_board_residue_alpha", 0.14) * _smoothstep(
            max(0.0, (progress - 0.18) / 0.82)
        )
    else:
...
def build_shell_preview_frame_for_state(
    ...
    *,
    ...
    if phase == _PHASE_RUPTURE:
        rupture_progress = shell_preview_timeline_progress(
            elapsed_ms, phase=_PHASE_RUPTURE, tuning=tuning
        )
        source_map = preview_source_cell_map(cache)
        impact_frames = tuple(
            (
                impact,
                _rupture_impact_draw_state(
                    impact, rupture_progress=rupture_progress, tuning=tuning
                ),
            )
    ...
    return EndgamePreviewFrame(
        ...
        impact_frames=tuple(
            ...
            for impact in cache.impacts
            if (
                draw_state := transform_boundary_impact(
                    impact, elapsed_ms=elapsed_ms, tuning=tuning
                )
            )
            is not None
        ),
        shard_frames=tuple(
            ...
            if (
                draw_state := transform_board_shard(
                    shard, elapsed_ms=elapsed_ms, tuning=tuning
                )
            )
            is not None
        ),
        residue_frames=_residue_frames(
            cache.impacts, elapsed_ms=elapsed_ms, tuning=tuning
        ),
    )
