...
def _draw_endgame_escaping_artifacts_3d(
    ...
    *,
    ...
    for artifact in endgame_animation.escaping_artifacts:
        ...
        pygame.draw.line(
            overlay,
            (*color, max(0, min(255, round(205 * alpha)))),
            start,
            end,
...
def _draw_endgame_board_shell_residue_3d(
    overlay: pygame.Surface,
    *,
    endgame_animation: EndgameAnimationState,
    dims: Cell3,
    ...
    for shell_fragment in endgame_animation.shell_fragments:
        ...
        start = project_raw_point_helper(start_point, dims, params, center_px)
        end = project_raw_point_helper(end_point, dims, params, center_px)
        if start is None or end is None:
            continue
        pygame.draw.line(overlay, color, start, end, 1)


def _draw_endgame_rupture_flash_3d(
    overlay: pygame.Surface,
    *,
...
def _draw_endgame_shell_fragments_3d(
    overlay: pygame.Surface,
    *,
    endgame_animation: EndgameAnimationState,
    dims: Cell3,
...
def _draw_clear_animation(
    surface: pygame.Surface,
    clear_anim: ClearAnimation3D | None,
    camera: Camera3D,
    center_px: Point2,
    dims: Cell3,
...
def _draw_endgame_board_3d(
    ...
    *,
    ...
    _draw_endgame_rupture_flash_3d(
        overlay,
        board_rect=board_rect,
        endgame_animation=endgame_animation,
    )
    _draw_endgame_board_shell_residue_3d(
        overlay,
        endgame_animation=endgame_animation,
        dims=dims,
        params=params,
    ...
    _draw_endgame_shell_fragments_3d(
        overlay,
        endgame_animation=endgame_animation,
        dims=dims,
        params=params,
            ...
            for segment in tuple(getattr(relic_state, "trail_segments", ())):
                ...
                pygame.draw.line(
                    ...
                    (
                        ...
                        color[1],
                        color[2],
                        max(0, min(255, round(176 * float(segment.alpha)))),
                    ),
                    tail,
                    head,
                    max(1, round(1.6 * max(0.35, float(segment.width)))),
                )
