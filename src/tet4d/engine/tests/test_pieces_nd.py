...
class TestPiecesND(unittest.TestCase):
    ...
    def test_3d_embedded_2d_shapes_have_zero_z(self):
        ...
        self.assertTrue(shapes)
        self.assertTrue(all(block[2] == 0 for shape in shapes for block in shape.blocks))
    ...
    def test_4d_embedded_3d_shapes_have_zero_w(self):
        ...
        self.assertTrue(shapes)
        self.assertTrue(any(any(block[2] != 0 for block in shape.blocks) for shape in shapes))
        self.assertTrue(all(block[3] == 0 for shape in shapes for block in shape.blocks))

    def test_4d_embedded_2d_shapes_have_zero_z_and_w(self):
        shapes = get_piece_shapes_nd(4, piece_set_id=PIECE_SET_4D_EMBED_2D)
        self.assertTrue(shapes)
        self.assertTrue(all(block[2] == 0 and block[3] == 0 for shape in shapes for block in shape.blocks))
    ...
    def test_random_piece_sets_are_seed_deterministic(self):
        ...
        shapes_3d_a = get_piece_shapes_nd(3, piece_set_id=PIECE_SET_3D_RANDOM, rng=rng_a)
        shapes_3d_b = get_piece_shapes_nd(3, piece_set_id=PIECE_SET_3D_RANDOM, rng=rng_b)
        self.assertEqual(
        ...
        shapes_4d_a = get_piece_shapes_nd(4, piece_set_id=PIECE_SET_4D_RANDOM, rng=rng_c)
        shapes_4d_b = get_piece_shapes_nd(4, piece_set_id=PIECE_SET_4D_RANDOM, rng=rng_d)
        self.assertEqual(
    ...
    def test_debug_4d_shapes_scale_with_board_size(self):
        ...
        self.assertGreater(large_counts["DBG_SURFACE_FLAT"], small_counts["DBG_SURFACE_FLAT"])
    ...
    def test_all_3d_piece_sets_have_no_zero_sized_pieces(self):
        for piece_set in PIECE_SET_3D_OPTIONS:
            shapes = get_piece_shapes_nd(3, piece_set_id=piece_set, board_dims=(10, 20, 6))
            ...
            for shape in shapes:
                ...
                self.assertTrue(all(span >= 1 for span in spans), f"zero span in 3D set: {piece_set}/{shape.name}")

    def test_all_4d_piece_sets_have_no_zero_sized_pieces(self):
        for piece_set in PIECE_SET_4D_OPTIONS:
            shapes = get_piece_shapes_nd(4, piece_set_id=piece_set, board_dims=(10, 20, 6, 4))
            ...
            for shape in shapes:
                ...
                for axis in range(4):
                    values = [block[axis] for block in shape.blocks]
                    spans.append(max(values) - min(values) + 1)
                self.assertTrue(all(span >= 1 for span in spans), f"zero span in 4D set: {piece_set}/{shape.name}")
