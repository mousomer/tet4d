...
Archived/supporting note: this audit captures a transitional ownership snapshot
around the 2026-03-14 cleanup passes. Current topology-playground architecture
authority lives in `docs/plans/topology_playground_current_authority.md`.
Newer task instructions and the current-authority manifest take precedence over
this audit. Use this file as supporting background rather than current
...
- At the time of this 2026-03-14 audit, the live explorer path presented four
  user-facing modes: `Edit`, `Inspect`, `Sandbox`, and `Play`. That
  intermediate mode model was later superseded by the accepted top-level
  `Editor` / `Sandbox` / `Play` contract recorded in
