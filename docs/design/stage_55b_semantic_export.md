...
probe versions as provenance. Generated wrapper identity is discarded using the
probe-reported `generated` flag alone; meaningful descendants survive. The flag
is the only signal, deliberately: `semantic_id` carries the full ancestor path,
so any identifier pattern match would also match every descendant and delete
real controls nested under a generated wrapper. It does not claim that every
runtime control has rich semantic annotation, and it has no per-screen
geometry, binding inventory, or layout policy.
...
The schemas remain intentionally separate: `tet4d.runtime-screen.v2` is the
...
rules; it does not require checked-in runtime captures. Fixture identifiers
reproduce the probe's real output shape, which is recorded next to them with
the Godot transformation that produced it, so a fixture cannot drift into a
form the probe never emits:
