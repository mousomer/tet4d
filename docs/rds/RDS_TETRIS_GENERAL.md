...
21x. The locked-cell explosion refactor must preserve the ownership split documented in `docs/plans/explosion_refactor_ownership_split.md`: shared board presentation under `src/tet4d/ui/pygame/board_presentation/` owns grid, shadow, boundary, trace, camera/view presentation inputs, W-movement, and dimensional board-view conventions; shared controls own dropdown/numeric/row-layout interaction semantics; shared explosion runtime owns particles, topology transport, bounce/collision, masses, elasticity, diagnostics, kinetic energy, trace samples, and runtime defaults; simulator, explorer, endgame, and gameplay remain thin launch/presentation adapters.
...
21k. Runtime explosion config assembly must route through one shared builder that combines persisted `ExplosionDefaults` with explicit launch-only runtime inputs such as topology, occupied cells, seed, and launch speed/time scaling. Simulator, explorer, and endgame adapters must all use that shared path rather than reassembling shared default fields independently.
21l. The explorer explosion adapter must remain thin: it may extract current topology/profile, sandbox/source cells, seed, and explorer-owned launch scaling/lifecycle text, but it must not duplicate shared defaults application, shared board presentation, or shared runtime behavior.
21m. Endgame must remain a thin shell layer around the shared explosion runtime/view. Rupture timing, noise/message sync, board-break chrome, debris release, gameplay locked-cell capture, topology capture, and live-subset selection are endgame-owned shell concerns; the persistent readable residue must reuse the shared explosion runtime/view entrypoint and saved simulator/shared defaults instead of a parallel endgame board-view/runtime implementation.
21n. Endgame must not release every locked board cell into live simulation by default. Instead, it must intentionally split into two populations: a shell-owned debris population for outward spectacle and a smaller persistent live-simulation population sized as `max(dimension_floor, round(endgame_live_cell_fraction * total_board_cell_count))`, capped by the number of available locked cells, with floors `20` in `2D`, `40` in `3D`, and `60` in `4D`; the default saved fraction is `0.12`. Non-selected locked cells must stay out of the live explosion simulation set, while gameplay topology and locked-cell sources remain the runtime handoff inputs.
21o. Gameplay board rendering must adopt the shared board-presentation layer incrementally. Stage 7A covers low-risk grid/shadow/boundary-box/edge-grid application, Stage 7B covers gameplay 4D W-movement style selection/application, and Stage 7C reuses only the low-risk camera/view presentation seams that are already clearly presentation-only, specifically 3D orthographic zoom-fit application and 4D frozen-view snapshot copying. Gameplay may still own rules/state, exposed presentation options, camera controls, and any entangled 4D basis/projection orchestration that is explicitly deferred, and these gameplay presentation passes must not route gameplay through simulator controls or explosion runtime code.
