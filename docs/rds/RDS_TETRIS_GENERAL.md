...
21o. Gameplay board rendering must adopt the shared board-presentation layer incrementally. Stage 7A covers low-risk grid/shadow/boundary-box/edge-grid application and Stage 7B covers gameplay 4D W-movement style selection/application; gameplay may still own rules/state, exposed presentation options, and camera/view behavior until Stage 7C, and these gameplay presentation passes must not route gameplay through simulator controls or explosion runtime code.
