...
21x. The locked-cell explosion refactor must preserve the ownership split documented in `docs/plans/explosion_refactor_ownership_split.md`: shared board presentation owns grid, shadow, boundary, trace, camera, W-movement, and dimensional board-view conventions; shared controls own dropdown/numeric/row-layout interaction semantics; shared explosion runtime owns particles, topology transport, bounce/collision, masses, elasticity, diagnostics, kinetic energy, trace samples, and runtime defaults; simulator, explorer, endgame, and gameplay remain thin launch/presentation adapters.
