...
18. The persistent relic phase must expose a real preset system with at least `wrap_all`, `invert_all`, and `sphere`, while keeping interaction semantics separate as `none` vs `collide`; collision support must not be encoded by multiplying preset ids.
...
20. Settings expose a dedicated `Endgame Effects` page for the persistent relic field, including the endgame preset selector and the separate interaction-mode selector, while `Game Settings` continues to own shared rotation animation mode, kick permissiveness (`kick_level`), and shared piece motion animation durations for rotation and deliberate translation tweens across 2D/3D/4D.
