...
The versioned interchange representation used by later native and Godot
migration is owned by `docs/architecture/canonical_topology_contract.md`.
That representation makes the existing Python paired-seam semantics explicit;
it does not transfer semantic authority or redefine Play drop/lock policy.
...
35. For topology-playground migration-state questions, `docs/architecture/topology_playground_current_authority.md` is the current architecture authority. Older topology-playground manifests and stage plans are historical unless explicitly reactivated.
...
11. Current topology-playground migration authority lives in `docs/architecture/topology_playground_current_authority.md`; older topology-playground manifests and stage plans are historical background only.
