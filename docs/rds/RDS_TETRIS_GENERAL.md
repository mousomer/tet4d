...
31. The Explorer Playground workspace model must be centered on `editor`, `sandbox`, and `play` both internally and in the visible top-level shell. Legacy `Inspect` and `Edit` labels may remain only as editor-scoped compatibility tool names, not as primary workspaces.
...
34. Editor trace visibility must be controlled by an explicit `on` / `off` button on the Explorer panel. Disabling trace must not hide or disable the editor probe/dot itself.
...
43. Explorer Playground helper text/panel selection must be keyed to the canonical workspace model (`editor` / `sandbox` / `play`). Editor helper content may vary by the active Editor tool, but legacy tool labels must not retake the primary top-level role.
44. The Explorer Playground scene shell must keep an explicit right-side helper panel visible outside the explorer viewport. That helper must stay concise for the topology-editor shell: minimal translation keys, minimal rotation keys, and at most one short workspace-context line.
