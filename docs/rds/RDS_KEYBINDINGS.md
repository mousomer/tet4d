...
4. Camera defaults are numeric-key based; macbook profile must provide a no-keypad fallback for all camera actions.
...
9. In Godot Live 4D, verify `view_xw/*` and `view_zw/*` change only the exact
...
9. 4D presentation-basis actions (`view_xw/*`,`view_zw/*`) are bindable and
