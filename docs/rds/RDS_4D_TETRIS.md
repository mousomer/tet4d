...
25. Active-piece animation must consume a frozen per-layer board-presentation snapshot for the full tween; basis-derived layer mapping, helper marks, projected grid primitives, layer-board anchor, zoom fit, and locked-cell projection must remain stable while only active-piece geometry changes over tween time. The renderer must build that frozen per-layer presentation once per move/tween start and reuse it across tween frames instead of rebuilding stable layer presentation work every frame.
26. Shared render-only projection-guide modes must exist in 4D: `bottom_boundary` projects the active piece onto the canonical gameplay floor plane, and `all_boundaries` projects it onto all relevant rendered board planes/layers for the current basis.
27. 4D projection guides must derive from active-piece render state against the frozen per-layer presentation, follow animated piece motion in gameplay and explorer rendering, and must not trigger per-frame stable layer-presentation rebuilds or change explorer non-safe traversal legality.
28. Terminal game over must enter `endgame_shatter` and then `endgame_relic_field`, keep the current basis/layer decomposition fixed, kill visible shell/frame fragments in a finite rupture, and hand visible locked cells to the dedicated seam-aware explosion subsystem as full-4D cell particles before projection without mutating live gameplay state. The frozen endgame basis may only affect the final projection/layer-board mapping, not the underlying ND particle motion state; connected seams must transform both position and velocity, non-connected boundaries must obey `boundary_response` (`escape` / `bounce`), and particle-particle resolution must obey the separate `particle_collisions` (`off` / `on`) axis.
