...
14. profile report via `tools/benchmarks/profile_4d_render.py` after projection/cache/zoom changes, with mitigation required if sparse overhead exceeds `15%` or `2.0 ms/frame`.
...
14. 4D render profiling tooling exists and is part of projection/cache change validation (`tools/benchmarks/profile_4d_render.py`).
