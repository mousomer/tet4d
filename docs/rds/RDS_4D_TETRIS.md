...
28. Terminal game over must enter `endgame_shatter` and then `endgame_relic_field`, keep the current basis/layer decomposition fixed, kill visible shell/frame fragments in a finite rupture, and carry visible locked-cell relic bodies through deterministic bounded preset-driven full-4D path families before projection without mutating live gameplay state; the frozen endgame basis may only affect the final projection/layer-board mapping, not the underlying relic motion state. The preset system must stay shared with `2D` / `3D`, including separate `none` / `collide` interaction semantics.
