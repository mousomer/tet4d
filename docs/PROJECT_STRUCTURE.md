...
- `src/tet4d/ai/playbot/planner_nd_search.py`: `SearchPlanND`, `plan_best_nd_with_budget(state, *, profile, planning_budget_ms, algorithm)`
...
- `src/tet4d/engine/gameplay/topology.py`: `normalize_topology_mode(mode)`, `topology_mode_from_index(index)`, `topology_mode_label(mode)`, `normalize_edge_behavior(value)`, `default_edge_rules_for_mode(axis_count, gravity_axis, *, mode, ...)`, `TopologyPolicy`, `map_piece_cells(policy, coords, *, allow_above_gravity)`, `map_overlay_cells(policy, coords, *, allow_above_gravity)`
...
- `src/tet4d/engine/runtime/menu_runtime_graph.py`: `MenuCompileResult`, `compile_runtime_menu_graph(authored_menus, *, authored_entrypoints)`, `validate_runtime_menu_graph(runtime_menus, *, runtime_entrypoints)`
...
- `src/tet4d/engine/runtime/runtime_config.py`: `gameplay_tuning_payload()`, `speed_curve_for_dimension(dimension)`, `challenge_prefill_ratio(dimension)`, `assist_bot_factor(mode_name)`, `assist_grid_factor(mode_name)`, `kick_level_names()`, `kick_default_level()`, `normalize_kick_level_name(level_name)`, `assist_kick_factor(level_name)`, `rotation_kick_candidate_offsets(level_name)`, `assist_speed_formula()`, `assist_combined_bounds()`, ...
...
- `src/tet4d/engine/runtime/settings_schema.py`: `RuntimeSettingDefaults`, `as_non_empty_string(value, *, path)`, `require_object(value, *, path)`, `require_list(value, *, path)`, `require_bool(value, *, path)`, `require_int(value, *, path, min_value=..., max_value=...)`, `require_number(value, *, path, min_value=..., max_value=...)`, `string_tuple(raw_values, *, path, normalize_lower=...)`, `mode_key_for_dimension(dimension)`, `clamp_overlay_transparency(value, *, default=...)`, `clamp_game_seed(value, *, default=...)`, `clamp_toggle_index(value, *, default=...)`, ...
...
- `src/tet4d/ui/pygame/board_presentation/gameplay_board.py`: `draw_gameplay_board_grid_2d(surface, *, board_rect, cell_size, width_cells, ...)`, `draw_gameplay_projection_segments_2d(surface, *, segments, board_offset, cell_size)`, `draw_gameplay_projected_grid(surface, *, dims, grid_mode, draw_full_grid, ...)`, `draw_gameplay_projection_faces(surface, *, faces)`, `resolve_and_draw_gameplay_occluded_board_lines(surface, *, board_line_primitives, ...)`, `draw_gameplay_over_piece_board_lines(surface, *, fragments, frame_color=..., ...)`, `apply_gameplay_orthographic_zoom_fit_3d(camera, *, dims, board_rect)`, `copy_gameplay_frozen_layer_view_4d(view)`, `normalize_gameplay_w_movement_style(style)`, `gameplay_w_movement_scale_for_layer(*, layer_distance, style)`
- `src/tet4d/ui/pygame/board_presentation/native_board.py`: `draw_native_board_view(surface, *, rect, fonts, controller, ...)`
...
- `src/tet4d/ui/pygame/endgame_animation.py`: `EndgamePresetConfig`, `EndgameAnimationTuning`, `load_endgame_animation_tuning()`, `EndgameRenderContext`, `SnapshotCell`, `EndgameSnapshot`, `ShellFragment`, `CellRelic`, `CellRelicMotionState`, `EndgameRelicRenderState`, `EndgameShatterState`, `EndgameDebrisReleaseState`, ...
...
- `src/tet4d/ui/pygame/locked_cell_explosion/defaults_store.py`: `clamp_endgame_live_cell_fraction(value)`, `ExplosionDefaults`, `default_explosion_defaults()`, `coerce_explosion_defaults(raw, *, defaults=...)`, `serialize_explosion_defaults(defaults)`, `mode_explosion_defaults(mode_key)`, `save_mode_explosion_defaults(mode_key, defaults)`, `save_shared_explosion_defaults(defaults)`
...
- `src/tet4d/ui/pygame/locked_cell_explosion/runtime_config.py`: `ExplosionRuntimeLaunchInputs`, `build_runtime_explosion_config(*, defaults, launch)`
...
- `src/tet4d/ui/pygame/topology_lab/explosion.py`: `ExplorerExplosionLaunchRequest`, `clear_scene_explosion(state)`, `consume_pending_scene_explosion_launch(state)`, `scene_explosion_particles(state)`, `build_sandbox_explosion_launch_request(state)`, `start_sandbox_explosion(state)`, `step_scene_explosion(state, *, dt_ms, play_sfx)`
...
- `src/tet4d/ui/pygame/locked_cell_explosion/runtime_config.py`: `tests/unit/engine/test_runtime_config.py` (exact)
