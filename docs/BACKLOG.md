...
`docs/architecture/work_type_classification.md` owns the accepted ontology and rollout. Workspace-governance `0.2.0` / `v0.2-work-type-schema-1` implemented the work-type vocabulary, artifact-role schema, complete pack roles and obligation comparators. `0.3.0` / `v0.3-role-resolution-1` moves role resolution into the pack, validates project role declarations, declares the pack's bootstrap roots and write-compatibility table, and adds the project `role_bootstrap` declaration.
Stage G1 provides the bounded, non-authoritative log-only observer (record schema 2): declared segment boundaries, repository-content write attribution, writes judged by the treatment captured at segment start, conditional compatibility with its named condition, candidate-role changes, bootstrap-root writes, and basic telemetry. It neither blocks nor performs the Tet4D semantic role handoff.
The 0.3.0 repair reopened the G1 deferral only to make the instrument trustworthy before evidence accrues; further governance work stays deferred while real product trajectories exercise G1. The non-self-relaxation comparison, human-reviewed Tet4D role migration (which must also classify `CURRENT_STATE.md`, `docs/CONFIGURATION_REFERENCE.md` and the work-type decision, now `unclassified`), stratified validation, and any separate Manifest change enabling blocking require a later evidence checkpoint and explicit approval. Separately open: register the work-type decision as a routed authority, and reconcile the policy pack's `governance_surface.file_classifications` with artifact roles.
