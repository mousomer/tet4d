...
**P0** (planning, recorded in those authorities): the pack records generic agent activity and experiments interpret it; capture is passive; per-run labels never live in `project.json`; snapshots are evidence, not attribution; raw-transcript retention is separable from telemetry retention. **P1a**: remove the pack's Tet4D literals (`REQUIRED_STABLE_AUTHORITIES`, `gov env`'s `TET4D_ENVIRONMENT_MODE`, the hardcoded project-manifest path), then add a versioned project-independent event schema, a log store outside the repository, and first-hand `gov` events. **P1b**: capture sources, meaning the Codex transcript importer moved out of the C1 experiment without its policy lookups, a Claude Code importer and hooks, and the G1 filesystem-snapshot core. **P1c**: generic reconstruction of segments, read/write timelines and invocation results.
