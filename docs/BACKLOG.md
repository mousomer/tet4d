...
- Fix strategy: make canonical selectors the only input authority when `scene_state.py` rebuilds runtime state, keep any remaining shell fields only as synchronized compatibility projections for legacy readers/diagnostics, route shell dirty-state through explicit helpers, and keep the legacy Normal Game rows/export path temporarily through the narrower transitional module `src/tet4d/ui/pygame/topology_lab/legacy_normal_mode_support.py` instead of the old catch-all file name.
