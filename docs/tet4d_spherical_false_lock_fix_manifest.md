...
## Purpose
Fix the real user-visible false-lock bug that still persists in topology playground **Play** mode on **spherical topology**.

This manifest exists because the earlier Stage 1 pass added useful structure and partial regression coverage, but did **not** capture the actual failing spherical case. The bug remains manually reproducible and must be treated as still open.
...
- The real **spherical** false-lock bug still persists in manual play.

### Therefore
- The existing automated tests are incomplete.
- The current branch must not claim that the Play false-lock bug is fully fixed.
- The next task is to derive a failing automated regression from the exact spherical repro and patch that runtime bug surgically.

---

## Problem statement

In topology playground **Play** mode on **spherical topology**, the piece can still lock incorrectly in a live user-visible session.

This is not yet considered solved.

The next work must answer:
