...
9. Architecture stage checkpoint (`arch_stage: 219`) additionally prepares and relocates launcher-play to `src/tet4d/ui/pygame/launcher_play.py`, canonicalizes CLI launcher imports, and records a zero-caller audit before final shim pruning while preserving `ui_to_engine_non_api = 0`.
