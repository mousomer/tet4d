...
9. Architecture stage checkpoint (`arch_stage: 204`) additionally prepares and relocates the shared ND loop runner to `src/tet4d/ui/pygame/loop_runner_nd.py`, canonicalizes 3D/4D engine callers, and records a zero-caller audit before final shim pruning while preserving `ui_to_engine_non_api = 0`.
