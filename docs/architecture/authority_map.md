...
| New Godot 2D/3D/4D/replay view and Live-4D presentation state | Godot/GDScript | Includes mode-owned camera orientation; the Stage 54C exact signed-axis basis; shared `SliceLocalOrientation`; centred point mapper; anchor-only layout; Stage 54E-2b renderer composition and corner-derived fit bounds; Stage 54E-2c shared-L interaction, `B + Q(L.local_yaw)` resolution, fitted reflection, actual Camera3D projection evidence, pitch policy, and preset compatibility adapter; and Stage 54E-2d's then-accepted entry/reset/teardown/re-entry implementation, deterministic/persistence exclusion, public roll removal, and internal basis-only reset. The accepted Stage 54E-4a forward contract refines presentation lifetime so same-context Restart/new game preserves the transient current view, one composite Reset View restores the complete mode-specific canonical view, Fit View reframes without changing orientation/basis/layout, and named presets are actions rather than persistent selected modes. It also defines flat canonical 2D and mode-owned replay reset/fit without applying Live-4D `B` or `L`. It excludes gameplay coordinates, legality, deterministic identity, and current-view persistence. Stages 54E-2a/b/c/d and aggregate Stage 54E-2 are complete and reviewed green; Stage 54E-4a closes the final blocking technical findings and awaits final confirmation review before implementation, with no remaining human decision. No authority transfer or establishment occurs. |
