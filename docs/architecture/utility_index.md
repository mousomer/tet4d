...
| Policy-pack serializer | `tools/governance/policy_pack_io.py` | Loads, validates, and writes the canonical compact policy representation. | Config/generated-data governance | Route every policy-pack writer and format check through this module. | Prevents tooling from obscuring machine-policy growth through incompatible formatting. |
...
| Artifact-role resolver | `tools/workspace_governance/resolver/roles.py` | Resolves declared and bootstrap-projected artifact roles, bootstrap roots, and write compatibility from loaded governance documents. | Workspace-governance pack | Obtain roles through `GovernanceResolver.role_index()`; never re-derive roles, roots, or the compatibility table outside the pack or from routes and paths. | Owns role semantics ahead of the Tet4D role migration and the non-self-relaxation comparison. |
| Agent telemetry | `tools/workspace_governance/telemetry.py` | Records first-hand `gov` invocations as versioned, project-independent events in an opt-in store outside the repository. | Workspace-governance pack | Record agent activity through this module and its event schema; keep interpretation and experiment logic outside it. | Keeps the execution record reusable when experiments change. |
| Work-segment observer | `tools/governance/observe_work_segment.py` | Records log-only segment boundaries, repository writes judged by the treatment captured at segment start, role provenance and changes, and bounded telemetry. | Governance tooling | Reuse its start/finish snapshots for observational work-type evidence; take roles and compatibility from the pack resolver. | Provides real-work evidence before any semantic role migration or blocking decision. |
...
| Godot persistent-file replacement | `godot/Tet4D.Godot/scripts/persistence/persistent_file_replacement.gd` | Checks sibling temporary writes and performs bounded destination/backup/install/restore mechanics, with sibling backup cleanup limited to an existing-destination replacement lifecycle. | Godot product-shell persistence mechanics | Reuse only for persistent Godot owners that need the same checked temp-write and rename/copy recovery guarantees; keep owner paths, schemas, diagnostics, semantic policy, and arbitrary fresh-destination sibling files outside the helper. | Prevents profile artifacts and ordinary shell settings from duplicating failure-prone file mechanics without merging their persistence domains or claiming unrelated backups. |
