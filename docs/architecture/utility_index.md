...
| Work-segment observer | `tools/governance/observe_work_segment.py` | Records log-only segment boundaries, repository writes, role provenance, compatibility, and bounded telemetry. | Governance tooling | Reuse its start/finish snapshots and exact role resolver for observational work-type evidence; do not derive roles from routes or paths. | Provides real-work evidence before any semantic role migration or blocking decision. |
