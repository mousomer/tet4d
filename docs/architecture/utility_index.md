...
| Godot persistent-file replacement | `godot/Tet4D.Godot/scripts/persistence/persistent_file_replacement.gd` | Checks sibling temporary writes and performs bounded destination/backup/install/restore mechanics. | Godot product-shell persistence mechanics | Reuse only for persistent Godot owners that need the same checked temp-write and rename/copy recovery guarantees; keep owner paths, schemas, diagnostics, and semantic policy outside the helper. | Prevents profile artifacts and ordinary shell settings from duplicating failure-prone file mechanics without merging their persistence domains. |
