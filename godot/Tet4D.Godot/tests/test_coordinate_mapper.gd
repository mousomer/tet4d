...
func run() -> Array:
	...
	_assert_vector(failures, bounds.get("max", Vector3.ZERO), Vector3(11.1, 3.5, 3.6), "bounds max includes active-spawn clearance")
	_assert_vector(failures, mapper.slice_label_position(1), Vector3(3.85, -3.7, -2.65), "W label position")
...
func _test_decomposed_asymmetric_mapping(failures: Array) -> void:
	...
	_assert_vector(failures, decomposition.get("anchor", Vector3.ZERO), Vector3(8.0, 0.0, 0.0), "asymmetric anchor_1")
	_assert_vector(failures, decomposition.get("unoriented_world_point", Vector3.ZERO), Vector3(7.0, 1.0, 1.0), "compatibility composition is G_D plus anchor")
